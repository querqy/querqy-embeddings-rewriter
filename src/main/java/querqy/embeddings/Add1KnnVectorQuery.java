...
public class Add1KnnVectorQuery extends Query {
     ...
     */
    public Add1KnnVectorQuery(String field, float[] target, int k) {
     ...
     * @throws IllegalArgumentException if <code>k</code> is less than 1
     */
    public Add1KnnVectorQuery(String field, float[] target, int k, Query filter) {
        this.field = field;
        this.target = target;
        this.k = k;
        if (k < 1) {
    ...
    public Query rewrite(IndexReader reader) throws IOException {
        ...
        Add1KnnVectorQuery.BitSetCollector filterCollector = null;
        if (filter != null) {
            filterCollector = new Add1KnnVectorQuery.BitSetCollector(reader.leaves().size());
    ...
    private TopDocs searchLeaf(LeafReaderContext ctx, Add1KnnVectorQuery.BitSetCollector filterCollector)
            throws IOException {
    ...
    public boolean equals(Object obj) {
        return sameClassAs(obj)
                && ((Add1KnnVectorQuery) obj).k == k
                && ((Add1KnnVectorQuery) obj).field.equals(field)
                && Arrays.equals(((Add1KnnVectorQuery) obj).target, target);
    }
